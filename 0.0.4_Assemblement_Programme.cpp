...
void SelectChannel(int n)
{
    ...
    for (unsigned char i = 0; i < 4; i++)
    {
        ...
        digitalWrite(CSADC_channel[i], HIGH); // Uniquement le CSADC à 1 de l'ADC dont on souhaite mofidier le channel
        ...
        //    digitalWrite(CSADC_channel[i], LOW); // Uniquement le CSADC à 1 de l'ADC dont on souhaite mofidier le channel
    }
...
long SpiRead(int ADC_CS_number, int channel_num)
{
    /*
    The LTC® 2424/LTC2428 are 4-/8-channel 2.7V to 5.5V micropower 
    20-bit A/D converters with an integrated oscillator, 
    8ppm INL and 1.2ppm RMS noise. They use delta-sigma technology 
    and provide single cycle digital filter settling time 
    (no latency delay) for multiplexed applications. 
    The first conversion after the channel is changed is always valid. 
    Through a single pin the LTC2424/ LTC2428 can be configured 
    for better than 110dB rejection at 50Hz or 60Hz ±2%, 
    or can be driven by an external oscillator for a user 
    defined rejection frequency in the range 1Hz to 800Hz. 
    The internal oscillator requires no external frequency 
    setting components

    fESCK: External SCK Frequency Range, 2000 kHz
    tCONV: Conversion Time,  160.23 ms
    tDOUT_ESCK: External SCK 24-Bit Data Output Time, 24/fESCK 
    t3: CS ↓ to SCK ↓, 150 ns

     LTC2428 State Transition Diagram : 
        CONVERT -> CHANNEL SELECTION -> SLEEP
         / \                               |
          |                               \ /
        DATA OUTPUT       <-      CSADC ↓ AND SCK ↓ ?
    ...
    */
    ...
    digitalWrite(CSADC_channel[ADC_CS_number], LOW); // take the SS pin low to select the chip
                                                     // probably not needed, only need 25 nsec delay
